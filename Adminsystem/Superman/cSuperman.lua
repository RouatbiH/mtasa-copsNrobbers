...
function Superman.Start()
	--if(isLoggedin())then
		--if(getElementData(lp,"duty:admin")==true)then
			local self = Superman
			...
			addEventHandler("onClientPlayerDamage", lp, Superman.onDamage, false)
			addEventHandler("onClientPlayerVehicleEnter",lp,Superman.onEnter)
			...
			--Check right
			triggerServerEvent("superman:checkRight",lp)
		--end
	--end
end
...
function showWarning()
	...
	local x,y,z = getPedBonePosition(lp,6)
...
function Superman.onEnter()
	...
	if (isPlayerFlying(lp) or getElementData(lp,"superman:takingOff")) and not isTimer(warningTimer) then
...
function Superman.onJump(key, keyState)
  ...
  if not isPlayerFlying(lp) then
	if task == "TASK_SIMPLE_IN_AIR" then
	  setElementVelocity(lp, 0, 0, TAKEOFF_VELOCITY)
...
function Superman.cmdSuperman()
  ...
  if isPedInVehicle(lp) or isPlayerFlying(lp) then return end
  setElementVelocity(lp, 0, 0, TAKEOFF_VELOCITY)
  setTimer(Superman.startFlight, TAKEOFF_FLIGHT_DELAY, 1)
  setElementData(lp,"superman:takingOff",true)
end

function Superman.startFlight()
  local self = Superman

  setElementData(lp,"superman:takingOff",false)
  if isPlayerFlying(lp) then return end

  triggerServerEvent("superman:start", rootElement)
  setPlayerFlying(lp, true)
  setElementVelocity(lp, 0, 0, 0)
  self.currentSpeed = 0
...
function Superman.processControls()
  ...
  if DirectionModule ~= 0 then
    ...
    if self.lastDirection then
      ...
      Direction = Vector3D:new(getElementVelocity(lp))
    end
  end
  Direction:Normalize()
  ...
  if self.currentSpeed > 0 then
    local VelocityDirection = Vector3D:new(getElementVelocity(lp))
  ...
  setElementVelocity(lp, Direction.x + self.extraVelocity.x,
...
function Superman:processIdleFlight(player)
  ...
  if player == lp then
    ...
    setPedRotation(lp, Sight.z)
    setElementRotation(lp, 0, 0, Sight.z)
  else
